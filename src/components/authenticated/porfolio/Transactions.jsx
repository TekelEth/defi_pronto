...
const Detail = ({ keys, value, shift }) => (
	<div className={`flex flex-col text-left ${shift && 'mr-[18%]'} items-start justify-self-start self-start`}>
		<span className='text-[#E74141] capitalize font-orbitron mb-1 text-[12px] leading-[14.4px]'>{keys}</span>
		...
		<div>
			<div className='px-6 bg-[#2F282E]  hidden md:block rounded-[20px] py-2'>
				<table className='w-full text-center   overflow-x-scroll'>
					<thead className='text-[#E74141] font-orbitron font-[400] border-b-[1px] border-[#353434]  text-[12px] leading-[14.4px]'>
						<td>Type</td>
						<td>Amount</td>
						<td>Currency</td>
						<td>Fee</td>
				...
				{transactions && transactions.length <= 0 && <NoContent message='Transaction' />}
			</div>
			<div className='px-6 bg-[#2F282E] md:hidden  block rounded-[20px] py-2'>
				{transactions &&
					transactions.length > 0 &&
					transactions.map((x, i) => (
						<Transaction key={i} amount={x.amount} address={x.address} type={x.type} status={x.status} />
					))}
			</div>
		</div>
	);
